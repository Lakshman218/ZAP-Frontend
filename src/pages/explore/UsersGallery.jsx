...
function UsersGallery({user}) {
  ...
  return (
    ...
    <div style={{width: '360px' }}>
      <div className="w-full group before:hover:scale-95 before:hover:h-80 before:hover:w-80 before:hover:h-44 before:hover:rounded-b-2xl before:transition-all before:duration-500 before:content-[''] before:w-80 before:h-20 before:rounded-t-2xl before:bg-gradient-to-bl from-sky-200 via-orange-100 to-slate-200 before:absolute before:top-0 w-80 h-64 relative bg-slate-50 flex flex-col items-center justify-center gap-2 text-center rounded-2xl overflow-hidden">
        <div className="w-24 h-24 bg-blue-700 mt-8 rounded-full border-4 border-slate-50 z-10 relative overflow-hidden">
          <img src={user.userId.profileImg} alt="Profile" className="w-full h-full object-cover rounded-full transition-transform duration-500 group-hover:scale-95" />
        </div>
        <div className="z-10 ">
          <span className="text-xl font-semibold">{user.userId.userName}</span>
        ...
        <Link 
          to={user.userId._id === userDataId
            ? "/profile"
            : `/user-profile/${user.userId._id}`}
          className="bg-blue-700 px-4 py-1 mb-2  text-slate-50 rounded-md z-10 hover:scale-125 transition-all duration-500 hover:bg-blue-500" href="#">
          View Profile
        </Link>
      </div>
    </div>
